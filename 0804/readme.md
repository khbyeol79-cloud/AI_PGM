...
### 14.8 한 프로젝트의 여러 `main()` 함수

`cal.cpp`, `cal1.cpp`, `cal2.cpp`, `ConsoleApplication3.cpp`에는 각각 `main()`이 있다. 여러 파일을 동시에 빌드 대상으로 지정하면 진입점이 중복되므로, 실행할 예제 하나만 빌드에 포함하거나 프로젝트를 각각 분리해야 한다.
...
18. 여러 `main()` 함수가 한 프로젝트에 포함되면 어떤 문제가 생기는가?
